...
def one_step_attention(a, s_prev):
    ...
    """
    s_prev = repeator(
        s_prev)   # 将s_prev进行重复输入，转化为(m, Tx, n_s)的形状，为了下一步与a_<t>合并做准备
    # 将s_<t-1>与a<t>合并成一个向量，准备计算注意力权重alpha_<t>
...
opt = Adam(
    # lr=0.005,
    beta_1=0.9, beta_2=0.999, decay=0.001)
model.compile(loss="categorical_crossentropy",
              optimizer=opt, metrics=["accuracy"]*10)
# 初始化s0 与 c0
s0 = np.zeros((m, n_s))
c0 = np.zeros((m, n_s))
...
# model.save_weights("models/weights.weights.h5")  # 保存权重信息
model.load_weights("models/weights.weights.h5")  # 加载权重信息
# 使用实际案例进行测试
...
for example in EXAMPLES:
    ...
    source = np.array(list(map(lambda x: to_categorical(
        x, num_classes=len(human_vocab)), source))).swapaxes(0, 1)
    source = source.transpose()  # 交互两轴
    source = np.expand_dims(source, axis=0)  # 增加一个轴
    print(source.reshape(1, 30, 37).shape,
          s0[:1].shape,
          c0[:1].shape)
